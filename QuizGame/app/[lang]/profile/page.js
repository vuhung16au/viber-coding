...
import { getAllCategories } from '@/app/firebase/database';
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (userSnapshot.exists()) {
          ...
          // Force re-fetch the current user to ensure we have fresh data
          ...
          console.log("No user data found in Firebase, using data from auth object");
          setProfile({
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Use the getAllCategories function to get categories from the database
        const categoriesData = await getAllCategories();
        // Get names of categories to use in the UI
                    ...
                    {profile.photoURL ? (
                      <img 
                        ...
                        onError={(e) => {
                          ...
                          e.target.src = "";
                          e.target.parentNode.innerHTML = `<span class="text-4xl text-gray-400">${profile.displayName?.[0]?.toUpperCase() || '?'}</span>`;
                        }}
                      />
                    ) : (
