...
export default function QuizzesCreatedLast30DaysPage({ params }) {
  const { lang } = params;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-400 mb-8">{t('last30DaysStatistics')}</p>
          ...
          <div className="bg-gray-700 p-4 rounded-lg">
            ...
            <p className="text-3xl font-bold">
              {stats?.quizzesCreated?.total > 0 
                ? Math.round((stats?.quizzesCreated?.last30Days / stats?.quizzesCreated?.total) * 100) 
                : 0}%
            </p>
          </div>
        </div>
